...
static struct stm_exp do_exp(T_exp exp) {
    if (exp->exp_type == T_BINOP) {
        return StmExp(
                reorder(canon_Exp_list(&exp->exp_value.BINOP.left, canon_Exp_list(&exp->exp_value.BINOP.right, NULL))),
                exp);
...
static T_stm do_stm(T_stm stm) {
    if (stm->stm_type == T_SEQ) {
        ...
        return seq(
                reorder(canon_Exp_list(&stm->stm_value.CJUMP.left, canon_Exp_list(&stm->stm_value.CJUMP.right, NULL))),
                stm);
        ...
        if (stm->stm_value.MOVE.dst->exp_type == T_TEMP && stm->stm_value.MOVE.src->exp_type == T_CALL)
            ...
            return seq(reorder(canon_Exp_list(&stm->stm_value.MOVE.dst->exp_value.MEM,
                                              canon_Exp_list(&stm->stm_value.MOVE.src, NULL))), stm);
