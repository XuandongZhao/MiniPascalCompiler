...
A_exp A_WhileExp(A_pos pos, A_exp test, A_exp body) {
    ...
    p->kind=A_whileExp;
    p->pos=pos;
    p->u._while.test=test;
    p->u._while.body=body;
    return p;
}
...
A_exp A_RepeatExp(A_pos pos, A_exp body, A_exp test) {
    ...
    p->kind=A_repeatExp;
    p->pos=pos;
    p->u._repeat.body=body;
    p->u._repeat.test=test;
    return p;
}
...
A_exp A_ArrayExp(A_pos pos, S_symbol typ, A_exp size, A_exp init) {
    ...
    p->kind=A_arrayExp;
    p->pos=pos;
    p->u._array._type=typ;
    p->u._array.size=size;
    p->u._array.init=init;
    return p;
}
...
A_ty A_ArrayTy(A_pos pos, A_ty range, S_symbol element) {
    ...
    printf("%d\t",pos);
    printf(element->name);
    p->u._array.range = range;
    p->u._array.element = element;
    return p;
}
