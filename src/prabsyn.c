...
void pr_exp(FILE *out, A_exp v, int d) {
    ...
    switch (v->kind) {
        ...
        case A_repeatExp:
            fprintf(out, "%s", "{\n");
            indent(out, d); fprintf(out, "\"name\": \"repeatExp\",\n");
            indent(out, d); fprintf(out, "\"children\": [\n");
            pr_exp(out, v->u._repeat.test, d+1); fprintf(out, ",\n");
            pr_exp(out, v->u._repeat.body, d+1);
            fprintf(out, "\n"); indent(out, d); fprintf(out, "]");
            fprintf(out, "\n"); indent(out, d); fprintf(out, "}");
            break;
        case A_breakExp:
...
static void pr_dec(FILE *out, A_dec v, int d) {
    ...
    switch (v->kind) {
        ...
        case A_constDec:
            fprintf(out, "%s", "{\n");
            indent(out, d); fprintf(out, "\"name\": \"constDec\",\n");
            indent(out, d); fprintf(out, "\"children\": [\n");
            pr_fundecList(out, v->u.function, d+1);
            fprintf(out, "\n"); indent(out, d); fprintf(out, "]");
            fprintf(out, "\n"); indent(out, d); fprintf(out, "}");
...
static void pr_ty(FILE *out, A_ty v, int d) {
    ...
    switch (v->kind) {
        case A_nameTy:
            ...
            break;
        case A_arrayTy:
            fprintf(out, "{\"name\": \"arrayTy(%s)\"}", S_name(v->u._array.element));
            break;
        default:
            assert(0);
    }
