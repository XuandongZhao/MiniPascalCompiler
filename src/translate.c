...
T_exp unEx(Tr_exp trexp) {
    if (trexp->tr_exp_type == Tr_ex) {
        ...
        return T_Eseq(temp_t_move, T_Eseq(trexp->tr_exp_storage.cx.stm, T_Eseq(T_Label(f),
...
T_stm unNx(Tr_exp trexp) {
    if (trexp->tr_exp_type == Tr_ex) {
        ...
        fillPatchList(temp_cx_falses, f);
        T_stm temp_t_move = T_Move(T_Temp(r), T_Const(1));
        return T_Exp(T_Eseq(temp_t_move, T_Eseq(trexp->tr_exp_storage.cx.stm, T_Eseq(T_Label(f),
...
Tr_exp Tr_forExp(Tr_exp lowerbound, Tr_exp upperbound, Tr_exp body, Tr_exp done, Tr_exp param) {
    ...
    T_exp temp_eseq = T_Eseq(params_add, T_Eseq(T_Label(test_l), T_Eseq(temp_cjump,
                                                                        T_Eseq(T_Label(unEx(done)->exp_value.NAME),
                                                                               T_Const(0)))));
...
Tr_exp Tr_whileExp(Tr_exp test, Tr_exp body, Tr_exp done) {
    ...
    T_exp temp_t_eseq = T_Eseq(T_Label(test_l),
                               T_Eseq(temp_t_cjump, T_Eseq(T_Label(unEx(done)->exp_value.NAME), T_Const(0))));
...
Tr_exp Tr_relExp(A_oper op, Tr_exp left, Tr_exp right) {
    ...
    switch (op) {
        case A_ltOp:
            cond = T_Cjump(T_lt, unEx(left), unEx(right), NULL, NULL);
            break;
        case A_leOp:
            cond = T_Cjump(T_le, unEx(left), unEx(right), NULL, NULL);
            break;
        case A_gtOp:
            cond = T_Cjump(T_gt, unEx(left), unEx(right), NULL, NULL);
            break;
        case A_geOp:
            cond = T_Cjump(T_ge, unEx(left), unEx(right), NULL, NULL);
            break;
        default:
            DEBUG_ERROR;
    }
...
Tr_exp Tr_ifExp(Tr_exp test, Tr_exp then, Tr_exp elsee) {
    ...
    if (elsee) {
        ...
        switch (then->tr_exp_type) {
            case Tr_nx:
                tmp = T_Seq(then->tr_exp_storage.nx, T_Label(f));
                break;
            case Tr_cx:
                tmp = T_Seq(then->tr_exp_storage.cx.stm, T_Label(f));
                break;
            default:
