...
#define INVALID_TYPE_COMPARISON(pos) EM_IRError(pos, "Semantic Error: Invalid Type Comparison")
...
SEM_expfrag SEM_AST2IRT(A_exp expr) {
    ...
    S_table type_env = E_base_type_env();
    S_table value_env = E_base_value_env();

    SEM_level = Tr_outermost();
    if (!SEM_level) {
        printf("###Not Inside Frag###\n");
    }

    expty tmp = transExp(value_env, type_env, expr); //翻译成IRTREE
    expfrag->exp = tmp.exp;
...
static expty transExp(S_table venv, S_table tenv, A_exp a) {
    ...
    switch (a->kind) {
        case A_opExp: {
            ...
            if (oper < 6) {
                if (left.ty->ty_type != Ty_int) {
                    EM_IRError(a->u._op.left->pos, "Semantic Error: The Left Operator Should Be Integer");
                } else if (right.ty->ty_type != Ty_int) {
                    EM_IRError(a->u._op.right->pos, "Semantic Error: The Right Operator Should Be Integer");
        ...
        case A_callExp: {
            string newmmp = malloc(sizeof(a->u._call.func->name) + 4);
            strcpy(newmmp, a->u._call.func->name);
            strcat(newmmp, "mmp");
            S_symbol mmpSym = S_Symbol(newmmp);
            E_enventry func_entry = S_look(venv, mmpSym);
            E_enventry callinfo = S_look(venv, mmpSym);
            ...
            if (callinfo && callinfo->env_type == E_funEntry &&
                ...
                return expTy(Tr_callExp(callinfo->env_storage.fun.label, callinfo->env_storage.fun.level, SEM_level,
                                        &argList),
                             Ty_UnwrapNamedTy(callinfo->env_storage.fun.result));
            } else {
                EM_IRError(a->pos, "Semantic Error: Undefined Function %s\n", S_name(a->u._call.func));
                return expTy(Tr_noExp(), Ty_Void());
            }
        }
        case A_arrayExp: {
            ...
            if (!array_ty) {
                EM_IRError(a->pos, "Semantic Error: Undeined Array %s", S_name(a->u._array._type));
                return expTy(Tr_noExp(), Ty_Int());
            }
            if (array_ty->ty_type != Ty_array) {
                EM_IRError(a->pos, "Semantic Error: %s Is Not An Array Type", S_name(a->u._array._type));
                return expTy(Tr_noExp(), Ty_Int());
            }
            ...
            if (array_size.ty->ty_type != Ty_int) {
                EM_IRError(a->pos, "Semantic Error: The Size Of Array Should Be Int");
                return expTy(Tr_noExp(), Ty_Int());
            } else if (!Ty_IsMatch(array_init.ty, array_ty->ty_storage.array)) {
                EM_IRError(a->pos, "Semantic Error: Array Type Unmatched %s", S_name(a->u._array._type));
                return expTy(Tr_noExp(), Ty_Int());
            } else {
        ...
        case A_forExp: {
            ...
            if (lowerbound.ty->ty_type != Ty_int) {
                EM_IRError(a->pos, "Semantic Error: The Lowerbound of For should be Int");
            } else if (upperbound.ty->ty_type != Ty_int) {
                EM_IRError(a->pos, "Semantic Error: The Upperbound of For should be Int");
            }
            ...
            return expTy(Tr_forExp(lowerbound.exp, upperbound.exp, body.exp, done, Tr_simpleVar(ac, SEM_level)),
                         Ty_Void());
        }
        case A_whileExp: {
            expty while_expty = transExp(venv, tenv, a->u._while.test);
            if (while_expty.ty->ty_type != Ty_int) {
                EM_IRError(a->pos, "Semantic Error: Testcase of While Should Be Int");
            }
            Tr_exp done = Tr_doneExp();
            SEM_break = done;
            SEM_break_count += 1;
        ...
        case A_repeatExp: {
            expty repeat_expty = transExp(venv, tenv, a->u._repeat.test);
            if (repeat_expty.ty->ty_type != Ty_int) {
                EM_IRError(a->pos, "Semantic Error: Testcase of Repeat Should Be Int");
            }
            Tr_exp done = Tr_doneExp();
            SEM_break = done;
            SEM_break_count += 1;
            expty body = transExp(venv, tenv, a->u._repeat.body);
            SEM_break = NULL;
            SEM_break_count -= 1;
            return expTy(Tr_whileExp(repeat_expty.exp, body.exp, done), Ty_Void());
        }
        case A_assignExp: {

            //处理函数返回值
            if (a->u._assign.var->isret == 1) {
            ...
            if (!Ty_IsMatch(assignvar_expty.ty, assignexp_expty.ty)) {
                EM_IRError(a->pos, "Semantic Error: Type of Left Unmatched Type of Right for Assign");
            }
                ...
                if (decs->head->kind == A_varDec) {
                    for (int i = 0; i != c; i++) {
                        if (strcmp(decNames[i], decs->head->u.var.var->name) == 0) {
                            EM_IRError(a->u._var->pos, "Semantic Error: Duplicate Variable Declaration Fault: %s",
                                       decNames[i]);
                        }
                    }
        ...
        case A_subscriptVar: {
            ...
            if (subscript_expty.ty->ty_type != Ty_array) {
                ...
                if (subexp_expty.ty->ty_type != Ty_int) {
                    EM_IRError(v->pos, "Semantic Error: Invalid Subscript Expr Type");
                    return expTy(Tr_noExp(), Ty_Int());
                } else {
...
static Tr_exp transDec(S_table venv, S_table tenv, A_dec d) {
    switch (d->kind) {
        case A_varDec: {
            printf("varDec");
            // 声明的时候赋值初始化
            if (strcmp(d->u.var._type->name, "integer") == 0) {
                d->u.var.init = (A_exp) malloc(sizeof(struct A_exp_));
                d->u.var.init->kind = 2;
                d->u.var.init->u._ival = 0;

            } else if (strcmp(d->u.var._type->name, "string") == 0) {
                d->u.var.init = (A_exp) malloc(sizeof(struct A_exp_));
                d->u.var.init->kind = 6;
                d->u.var.init->u._sval = "";
            }
        ...
        case A_functionDec: {
            ...
            for (A_fundecList fcl = d->u.function; fcl; fcl = fcl->tail) {
                ...
                if (fcl->head->result) {
                    ...
                    if (!result_ty) {
                        EM_IRError(fcl->head->pos, "Semantic Error: Undefined Return Type");
                    }
                }
                ...
                for (A_fieldList fl = fcl->head->params; fl; fl = fl->tail) {
                    ...
                    if (!ty) {
                        EM_IRError(fl->head->pos, "Semantic Error: Undefined Type: %s", S_name(fl->head->_type));
                        ty = Ty_Int();
                    }
                    if (!tmp) {
                ...
                //新建mmp变量，加入venv
                Tr_access ac = Tr_allocLocal(SEM_level, FALSE); //TODO：我也不知道它能不能访问啊orz
                A_var copy = (A_var) malloc(sizeof(struct A_var_));
                string oldname = fcl->head->name->name;
                copy->u.simple = S_Symbol(oldname);
                //copy->kind = fcl->head->result;
                copy->isret = 1;
                S_enter(venv, copy->u.simple, E_addVarEntry(ac, Ty_Int()));
                string c = malloc(sizeof(strlen(oldname) + 4));
                strcpy(c, oldname);
                strcat(c, "mmp");
                S_symbol funcSym = S_Symbol(c);
                S_enter(venv, funcSym, E_addFunEntry(new_level, func_label, formal_ty, result_ty));
            }

            for (A_fundecList fcl = d->u.function; fcl; fcl = fcl->tail) {
                A_fundec func_dec = fcl->head;
                string newmmp = malloc(sizeof(func_dec->name->name) + 4);
                strcpy(newmmp, func_dec->name->name);
                strcat(newmmp, "mmp");
                S_symbol mmpSym = S_Symbol(newmmp);
//                S_printSymTable();
                E_enventry func_entry = S_look(venv, mmpSym);
                ...
                for (A_fieldList l = func_dec->params;
                     l && start && acl; l = l->tail, start = start->tail, acl = acl->tail) {
                ...
                body_expty.ty = func->env_storage.fun.result;
        ...
        case A_typeDec: {
            ...
            for (A_nametyList nl = d->u._type; nl; nl = nl->tail) {
                ...
                switch (nl->head->ty->kind) {
                    case A_nameTy: {
                        ...
                        if (!res_ty) {
                            EM_IRError(nl->head->ty->pos, "Semantic Error: Undefined Type %s",
                                       S_name(nl->head->ty->u.name));
                            res_ty = Ty_Int();
                        }
                        break;
                    }
                    case A_arrayTy: {
                        Ty_ty tmp = S_look(tenv, nl->head->ty->u._array.element); //TODO:？？？？
                        if (!tmp) {
                            EM_IRError(nl->head->ty->pos, "Semantic Error: Undefined Type %s",
                                       S_name(nl->head->ty->u._array.element));//TODO:？？？？
                        }
                        res_ty = Ty_Array(tmp);
                        break;
                    }
            ...
            if (iscyl)
                EM_IRError(d->pos, "Semantic Error: Illegal Type Cycle");
            return Tr_noExp();
        }
        default:
            DEBUG_ERROR;
...
static inline bool SEM_argmatch(S_table venv, S_table tenv, A_expList exp_list, Ty_tyList arg_list, A_exp fun) {
    ...
    while (exp_l && arg_l) {
        ...
        if (!Ty_IsMatch(t.ty, arg_l->head)) {
            EM_IRError(fun->pos, "Semantic Error: Params Unmatched in Function %s\n", S_name(fun->u._call.func));
            return FALSE;
        }
        exp_l = exp_l->tail;
        arg_l = arg_l->tail;
    ...
    if ((exp_l && !arg_l) || (!exp_l && arg_l)) {
        EM_IRError(fun->pos, "Semantic Error: Params Unmatched");
        return FALSE;
    } else
        return TRUE;
}