...
void printStmJson(FILE *out, T_stm stm, int d) {
    ...
    switch (stm->stm_type) {
        case T_SEQ: {
            fprintf(out, "%s", "{\n");
            indent(out, d);
            fprintf(out, "\"name\": \"SEQ\",\n");
            indent(out, d);
            fprintf(out, "\"children\": [\n");
            printStmJson(out, stm->stm_value.SEQ.left, d + 1);
            fprintf(out, ",\n");
        ...
        case T_CJUMP: {
            fprintf(out, "%s", "{\n");
            indent(out, d);
            fprintf(out, "\"name\": \"CJUMP\",\n");
            indent(out, d);
            fprintf(out, "\"children\": [\n");
            indent(out, d + 1);
            fprintf(out, "{\"name\": \"%s\"},\n", rel_op[stm->stm_value.CJUMP.op]);
            printTreeJson(out, stm->stm_value.CJUMP.left, d + 1);
            fprintf(out, ",\n");
            printTreeJson(out, stm->stm_value.CJUMP.right, d + 1);
            fprintf(out, ",\n");
        ...
        case T_MOVE: {
            fprintf(out, "%s", "{\n");
            indent(out, d);
            fprintf(out, "\"name\": \"MOVE\",\n");
            indent(out, d);
            fprintf(out, "\"children\": [\n");
            printTreeJson(out, stm->stm_value.MOVE.dst, d + 1);
            fprintf(out, ",\n");
...
void printTreeJson(FILE *out, T_exp exp, int d) {
    ...
    switch (exp->exp_type) {
        case T_BINOP: {
            ...
            printTreeJson(out, exp->exp_value.BINOP.left, d + 1);
            fprintf(out, ",\n");
        ...
        case T_ESEQ: {
            fprintf(out, "%s", "{\n");
            indent(out, d);
            fprintf(out, "\"name\": \"ESEQ\",\n");
            indent(out, d);
            fprintf(out, "\"children\": [\n");
            printStmJson(out, exp->exp_value.ESEQ.stm, d + 1);
            fprintf(out, ",\n");
